...
func place_camera(exitDoor):
	...
	$Spatial.transform = $Spatial.transform.rotated(Vector3(0, 0, 1), (-get_parent().rotation_degrees.x) / 180 * PI)
	$Spatial.transform = $Spatial.transform.rotated(Vector3(0, 1, 0), (-get_parent().rotation_degrees.y) / 180 * PI)
	$Spatial.transform = $Spatial.transform.rotated(Vector3(1, 0, 0), (-get_parent().rotation_degrees.z) / 180 * PI)
	$Spatial.translation += Vector3(0, 1, 0)
	$Spatial.transform = $Spatial.transform.looking_at(Vector3(0, 3, 0), Vector3(0, 1, 0))
	$Spatial.transform = $Spatial.transform.rotated(Vector3(0, 0, 1), (exitDoor.rotation_degrees.x) / 180 * PI)
	$Spatial.transform = $Spatial.transform.rotated(Vector3(0, 1, 0), (180 + exitDoor.rotation_degrees.y) / 180 * PI)
	$Spatial.transform = $Spatial.transform.rotated(Vector3(1, 0, 0), (exitDoor.rotation_degrees.z) / 180 * PI)
