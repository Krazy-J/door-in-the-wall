...
func _ready():
	if not Engine.editor_hint and door:
		...
		if section[0].locked:
			$Frame.material = load("res://assets/material/plastic/red.tres")
			set_locked(true)
			...
			$Frame.material = load("res://assets/material/medal/gold.tres")
			exit.get_node("Frame").material_override = load("res://assets/material/medal/gold.tres")
		elif section_completed >= section.size() * .75 - 1:
			$Frame.material = load("res://assets/material/medal/silver.tres")
			exit.get_node("Frame").material = load("res://assets/material/medal/silver.tres")
		elif section_completed >= section.size() * .5 - 1:
			$Frame.material = load("res://assets/material/medal/bronze.tres")
			exit.get_node("Frame").material = load("res://assets/material/medal/bronze.tres")
