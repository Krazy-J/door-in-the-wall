...
func place_camera():
	...
	$Viewport/Spatial/Camera.near = max(0.1, ($Viewport/Spatial.global_transform.origin - global_transform.origin).rotated(Vector3(0, 1, 0), -$Viewport/Spatial.rotation_degrees.y / 180 * PI).z - 1) * exit_door.scale.y / scale.y
