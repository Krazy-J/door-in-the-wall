...
func _ready():
	...
	get_child(0).shape.radius = size / 2.3
	get_child(0).shape.height = $Ring.outer_radius - size / 2
	for collision in range(6):
		get_child(collision).shape = get_child(0).shape
		get_child(collision).transform = Transform(Vector3(0,1,0),Vector3(-1,0,0),Vector3(0,0,1), Vector3(0, 0, radius + size / 6)).rotated(Vector3(0,-1,0), collision / 3.0 * PI)
